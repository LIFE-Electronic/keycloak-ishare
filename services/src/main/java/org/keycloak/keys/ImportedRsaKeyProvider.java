...
public class ImportedRsaKeyProvider extends AbstractRsaKeyProvider {
    ...
    public KeyWrapper loadKey(RealmModel realm, ComponentModel model) {
        ...
        List<X509Certificate> certificateChain = PemUtils.decodeCertificateChain(certificatePem);

        KeyUse keyUse = KeyUse.valueOf(model.get(Attributes.KEY_USE, KeyUse.SIG.name()).toUpperCase());

        return createKeyWrapper(keyPair, certificate, certificateChain, keyUse);
    }
